...
exports.notifySlackWithResults = exports.getAllureReportUrl = void 0;
...
function getAllureReportUrl() {
    return new Promise((resolve, reject) => {
        const allureProcess = (0, child_process_1.spawn)("allure", ["serve", "allure/allure-results"]);
...
exports.getAllureReportUrl = getAllureReportUrl;
function calculateTrend(currentPassed, currentTotal) {
    const historyPath = path_1.default.resolve('allure/allure-history/last-summary.json');
    if (!fs_1.default.existsSync(historyPath)) {
        fs_1.default.writeFileSync(historyPath, JSON.stringify({ passed: currentPassed, total: currentTotal }, null, 2));
        return "🆕 *First recorded test run — baseline created!*";
    }
    const lastSummary = JSON.parse(fs_1.default.readFileSync(historyPath, 'utf8'));
...
function getFailedTests() {
    const dataDir = path_1.default.resolve('allure/allure-results');
    ...
    for (const file of files) {
        ...
        try {
            const testResult = JSON.parse(fs_1.default.readFileSync(filePath, 'utf8'));
            if (testResult.status === "failed") {
                const testId = Object.keys(test_metadata_1.testMetadata).find(key => testResult.name.includes(test_metadata_1.testMetadata[key].displayname));
                if (testId && !failedTestsSet.has(testResult.name)) {
                    failedTestsSet.add(testResult.name);
                    failedTestsList.push({
                        title: `[${test_metadata_1.testMetadata[testId].tms}] ${testResult.name}`,
                        status: "failed",
                        tmsId: test_metadata_1.testMetadata[testId].tms,
                        tmsUrl: `https://tobiasa-team.atlassian.net/browse/${test_metadata_1.testMetadata[testId].tms}`
                    });
                }
            }
        }
    ...
    fs_1.default.writeFileSync('test-reports.json', JSON.stringify(failedTestsList, null, 2));
    return failedTestsList;
}
function notifySlackWithResults(allureReportUrl) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        const summaryPath = path_1.default.resolve('allure/allure-report/widgets/summary.json');
        ...
        const response = yield (0, node_fetch_1.default)("https://slack.com/api/chat.postMessage", {
            method: "POST",
            headers: {
...
exports.notifySlackWithResults = notifySlackWithResults;
//# sourceMappingURL=slack-manager.js.map