 ...
 */
export async function connectToDatabase(targetDB: string, dbType: 'timescale' | 'postgresql') {
    const dbConfig = getDBConfig(targetDB);
 ...
 */
export async function switchTestDatabase(index: number) {
    if (![1, 2].includes(index)) {
        throw new Error(`Invalid database index: ${index}. Expected 1 or 2.`);
    }

    const dbName = index === 1 ? TEST_DB_1 : TEST_DB_2;
    await DBManager.connect('timescale', getDBConfig(dbName));
}
...
export async function getFinancialDataRowCount() {
...
export async function getFinancialDataColumnNames() {
    const result = await DBManager.getPGClient().query(`
...
export async function getFinancialDataFirstRow() {
    const result = await DBManager.getPGClient().query(`
...
export async function getAverageFinancialDataValue() {
    const result = await DBManager.getPGClient().query(`
...
export async function getTotalDataValueForQuarter(quarter: string) {
    const quarterMonths = {
...
export async function findMissingPeriodsForSeries(seriesReference: string, year: string) {
...
export async function verifyDataValueTrend() {
    const result = await DBManager.getPGClient().query(`
...
export async function validateDataRange(minValue: number, maxValue: number) {
    const result = await DBManager.getPGClient().query(`
...
export async function checkDuplicateEntries() {
    const result = await DBManager.getPGClient().query(`
        SELECT series_reference, period, COUNT(*) 
        FROM business_financial_data
        GROUP BY series_reference, period
...
export async function verifyDataConsistencyAcrossPeriods() {
    const result = await DBManager.getPGClient().query(`
...
export async function getFinancialDataRange() {
    const result = await DBManager.getPGClient().query(`
...
export async function getRetainedDataCount() {
    const result = await DBManager.getPGClient().query(`
        SELECT COUNT(*) AS count FROM business_financial_data
...
export async function getDataRangeForSubject(subject: string) {
    const result = await DBManager.getPGClient().query(`
...
export async function hasGapsInPeriods() {
    const result = await DBManager.getPGClient().query(`
        WITH sorted_data AS (
...
export async function getMissingDataBySeries() {
    const result = await DBManager.getPGClient().query(`
...
export async function checkFinancialDataTrends() {
    const result = await DBManager.getPGClient().query(`
        WITH trend_data AS (
...
export async function detectAnomalousSpikes() {
    const result = await DBManager.getPGClient().query(`
        WITH stats AS (
